...
local ANALYTICS_URL  = "https://cyrus-hub-backend.vercel.app/api/analytics"
local ANALYTICS_SEC  = "CYRUS_SEC_9kZmX3pQ7wR2nL5v"
...
--// =========================================
--//   FETCH JSON (com timeout — não bloqueia)
--// =========================================
...
local GAMES = {
    ...
    [2788229376]="DaHood",
    -- King's Legacy
    [3214529440]="KingLegacy",[8173705722]="KingLegacy",[4520749081]="KingLegacy",
    -- Weak Legacy
    [7054676802]="WeakLegacy",
    -- Anime Fighting / Warriors
    [921042218]="AnimeFighting",[5567483462]="AnimeFighting",[18923620224]="AnimeFighting",
    -- Jujutsu Shenanigans
    [13049847188]="JujutsuShenanigans",[9391468976]="JujutsuShenanigans",
    -- Blade Ball
    ...
    -- Shindo Life
    [6372028509]="ShindoLife",
    -- GPO
    [8737384332]="GPO",
    -- Fruit Battlegrounds
    [13022880397]="FruitBattlegrounds",
    -- Anime Adventures / Apocalypse
    [12282861865]="AnimeAdventures",[140409475718339]="AnimeAdventures",
    -- Slime RNG / Sol's RNG
    [17017769292]="SolsRNG",[92416421522960]="SolsRNG",
    -- Type Soul
    [9504325490]="TypeSoul",
    -- BBN / STA
    [8202280624]="BBN",[9098570654]="STA",
    -- Monkey Bomb Tag
    [108843562394237]="MonkeyBombTag",
    -- Kick a Lucky Block
    [89469502395769]="KickLuckyBlock",
    -- Be a Flash for Brainrots
    [136066387156306]="BeFlashBrainrots",
    -- Attack on Titan Revolution
    [13379208636]="AOTRevolution",
    -- Blue Lock: Rivals
    [18668065416]="BlueLockRivals",
    -- Build a Ring Farm
    [107646426076756]="BuildRingFarm",
    -- District of Violence
    [93978595733734]="DistrictViolence",
    -- Split or Steal Brainrot
    [70959844973091]="SplitStealBrainrot",
    -- The Strongest Battlefields
    [10449761463]="StrongestBattlefields",
}
...
local function sendAnalytics(accessType)
    pcall(function()
        ...
        local function getCountry2()
            local ok,loc=pcall(function() return Players.LocalPlayer.LocaleId or "" end)
            if ok and loc~="" then
                local m={
                    ["pt-br"]="Brasil",["en-us"]="Estados Unidos",["en-gb"]="Reino Unido",
                    ["es-es"]="Espanha",["ru-ru"]="Russia",["vi-vn"]="Vietna",
                    ["th-th"]="Tailandia",["tr-tr"]="Turquia",["fr-fr"]="Franca",
                    ["de-de"]="Alemanha",["id-id"]="Indonesia",["ko-kr"]="Coreia do Sul",
                    ["ja-jp"]="Japao",["ar-sa"]="Arabia Saudita",["pl-pl"]="Polonia",
                }
                return m[loc:lower()] or loc
            end
            return "Desconhecido"
        end
        ...
        local reqData = {
            Url    = ANALYTICS_URL,
            Method = "POST",
            Headers = {["Content-Type"]="application/json",["X-Cyrus-Token"]=ANALYTICS_SEC},
            Body   = payload,
        }
        if syn and syn.request then syn.request(reqData)
        elseif http and http.request then http.request(reqData)
        elseif request then request(reqData)
        else game:HttpPost(ANALYTICS_URL, payload, false, "application/json") end
    end)
end
...
do
    ...
    if temPremium or temKeyValida or kernelOff then
        ...
        elseif temKeyValida then
            session.type="12h"; session.expiresAt=savedKey and savedKey.time and (savedKey.time+43200) or nil
